...
const server = setupServer(
  ...
  http.put('http://localhost:8787/api/todos/:id', async (info) => {
    ...
    const body = await info.request.json() as { text?: string; completed?: boolean };
...
describe('API Client Integration', () => {
  ...
  describe('Health Check', () => {
    it('should successfully check API health', async () => {
      ...
      const data = await response.json() as { status: string; message: string; timestamp: string };
  ...
  describe('Todo Operations', () => {
    it('should fetch all todos', async () => {
      ...
      const todos = await response.json() as Array<{ id: number; text: string; completed: boolean }>;
    ...
    it('should create a new todo', async () => {
      ...
      const todo = await response.json() as { id: number; text: string; completed: boolean };
